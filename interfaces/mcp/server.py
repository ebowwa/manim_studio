...
# Import avoiding the circular dependency issues
try:
...
class ManimStudioServer:
    ...
    async def execute_tool(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if name == "create_scene":
            ...
            try:
                # Render using manim directly
                import subprocess
                cmd = ["manim", temp_path, self.current_scene['name'], "-p", f"--quality={quality}_quality"]
            ...
            try:
                # Render using manim directly
                import subprocess
                cmd = ["manim", temp_path, self.current_scene['name'], f"--quality={quality}_quality", "-o", output_path]
                subprocess.run(cmd, check=True)
                Path(temp_path).unlink()  # Clean up temp file
                return {
                    "status": "success",
